...
# 🚀 FINAL FIX: Updated list of domains for more reliable filtering
DOMAINS = ["Any", "Computer Science", "Physics", "Mathematics", "Biology", "Medicine", "Social Science", "Engineering", "Arts and Humanities"]
...
def unified_search(query: str, year_range: Tuple[int,int], domain_filter: str, 
    ...
    try:
        ...
        # Initialize api_times for the new search
        st.session_state.api_times = {}

        with st.spinner("Searching academic databases..."):
            for source in sources:
                if source in source_fetchers:
                    start_time = time.time() # Start timing for the current source
                    with st.spinner(f"Searching {source.upper()}..."):
                        try:
                            results += source_fetchers[source](processed_query, RESULTS_PER_SOURCE)
                            end_time = time.time()
                            st.session_state.api_times[source] = end_time - start_time
                        except Exception as e:
                            # Silently handle errors
                            print(f"Error fetching from {source.upper()}: {e}")
                            end_time = time.time()
                            st.session_state.api_times[source] = end_time - start_time # Log even if it fails

        # 🚀 NEW: Store the count of raw results before deduplication
        st.session_state.raw_results_count = len(results)
...
if "citations_range" not in st.session_state:
    # 🚀 FIX: Increase the default max value for the citations range slider.
    st.session_state.citations_range = (0, 10000)
...
# 🚀 NEW: Initialize session state for performance metrics
if "total_search_time" not in st.session_state:
    st.session_state.total_search_time = 0.0
# 🚀 NEW: Initialize session state for raw results count
if "raw_results_count" not in st.session_state:
    st.session_state.raw_results_count = 0
...
def run_search_with_query():
    ...
    if st.session_state.current_query and st.session_state.sources:
        ...
        # 🚀 NEW: Stop total search timer
        end_total = time.time()
        st.session_state.total_search_time = end_total - start_total
...
with st.sidebar:
    ...
    # 🚀 FIX: Update the max_value of the slider to 10000
    st.slider("Citations range", min_value=0, max_value=10000, 
...
if not st.session_state.similar_papers_df.empty:
    ...
    for i, row in st.session_state.similar_papers_df.iterrows():
        with st.container():
            ...
            with col1:
                # 🚀 FIX: Use HTML <b> tag for bolding instead of markdown **
                st.markdown(f'<div class="paper-meta"><b>✍️ Authors:</b> {row.get("authors","N/A") or "N/A"}</div>', unsafe_allow_html=True)
            with col2:
                # 🚀 FIX: Use HTML <b> tag for bolding instead of markdown **
                year_display = safe_int(row.get("year"))
                st.markdown(f'<div class="paper-meta"><b>📅 Year:</b> {year_display or "N/A"}</div>', unsafe_allow_html=True)
            with col3:
                # 🚀 FIX: Use HTML <b> tag for bolding instead of markdown **
                st.markdown(f'<div class="paper-meta"><b>📈 Citations:</b> {row.get("citation_count", 0) or 0}</div>', unsafe_allow_html=True)
            
            # 🚀 FIX: Use a simple string for the expander label, Streamlit handles the icon
            ...
            # 🚀 FIX: Use HTML <b> tag for bolding instead of markdown **
            st.markdown(f"<b>Source:</b> {row.get('source','').upper()} &nbsp; • &nbsp; <b>Type:</b> {row.get('publication_type','N/A') or 'N/A'}", unsafe_allow_html=True)
            st.markdown(f"<b>Categories:</b> <i>{row['categories'] or 'N/A'}</i>", unsafe_allow_html=True)
            ...
            if "hybrid_score" in row:
                ...
                # 🚀 FIX: Use HTML <b> tag for bolding instead of markdown **
                st.markdown(f"<b>Final Score:</b> `{score:.2f}` (Similarity: `{sim_score:.2f}`)", unsafe_allow_html=True)

            st.markdown("---")
            if st.button(f"🔍 Find Similar Papers", key=f"similar_btn_{i}"):
    ...
    # 🚀 FIX: Added back the button to return to the main search results
...
# Display main results
    ...
    # 🚀 FIX: Re-adding Year Span metric
    ...
    with perf_col1:
        st.metric("Total Search Time", f"{st.session_state.total_search_time:.2f}s" if "total_search_time" in st.session_state else "N/A")
    with perf_col2:
        # 🚀 NEW: Display raw vs. unique papers metric
        if st.session_state.raw_results_count > 0:
            st.metric("Raw vs. Unique Papers", f"{len(st.session_state.results_df)} of {st.session_state.raw_results_count} found")
        else:
            st.metric("Raw vs. Unique Papers", "N/A")
        ...
        if st.session_state.filtered_df.empty:
            ...
            for i, row in st.session_state.filtered_df.iterrows():
                with st.container():
                    ...
                    # 🚀 FIX: Pass a clean string to st.expander, as it doesn't parse HTML in the label
                    with st.expander("📖 Show Abstract"):
                        abstract = row["abstract"] or "No abstract available."
                        st.markdown(f'<div class="paper-abstract">{abstract}</div>', unsafe_allow_html=True)
